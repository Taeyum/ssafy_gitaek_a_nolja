          ...
          <div class="absolute top-6 left-6 right-6 z-10 flex flex-col gap-3 pointer-events-none">
            ...
            <div class="pointer-events-auto flex justify-end gap-2 max-w-xl">
               ...
               >
                 ...
                 <span class="bg-gradient-to-r from-purple-600 to-pink-600 bg-clip-text text-transparent">AI 코스 추천</span>
               </button>
            </div>
          ...
          <div class="space-y-5">
            <div>
              ...
              <input 
                ...
                :placeholder="tripStore.tripInfo?.title || '도시 이름을 입력하세요 (예: 강릉)'" 
              />
            ...
            <div>
               <label class="block text-sm font-bold text-gray-500 mb-1">여행 기간</label>
               <div class="w-full border p-3 rounded-xl bg-gray-100 text-gray-500 flex justify-between items-center cursor-not-allowed">
                  <span class="text-sm">{{ tripStore.tripInfo?.startDate }} ~ {{ tripStore.tripInfo?.endDate }}</span>
                  <span class="font-bold text-[#DE2E5F]">총 {{ aiForm.totalDays }}일</span>
               </div>
            </div>
  
            <div>
               <label class="block text-sm font-bold text-gray-800 mb-1">어떤 여행을 원하세요?</label>
               <input 
                 ...
                 @keyup.enter="fetchAiPlan" 
               />
            </div>
          </div>
  
          <div class="mt-8 flex gap-3">
            <button @click="showAiModal = false" class="flex-1 py-3.5 bg-gray-100 rounded-xl font-bold text-gray-600 hover:bg-gray-200 transition-colors">취소</button>
            <button 
              @click="fetchAiPlan" 
